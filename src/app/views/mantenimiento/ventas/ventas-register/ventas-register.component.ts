...
export class VentasRegisterComponent implements OnInit {
  ...
  changerValueFormArray(i: number) {
    ...
    if (obj_1.cantidad < obj_1.stock) {
      let precio_total =
        obj_1.cantidad * obj_1.precio_unitario - obj_1.descuento;
      ...
      Swal.fire({
        position: 'center',
        icon: 'info',
        title: 'Ingrese una cantidad menor',
        showConfirmButton: false,
        timer: 1650,
      });
      let obj = this.DetalleVentas.controls[i].get('cantidad')?.setValue(1);
       obj = this.DetalleVentas.controls[i].get('precio_total')?.setValue(0);
    }
